...
void Network::fd2Save(string namefile)
{
    mat * r;
    ...
    int sizeLast= vectLayer->at(vectLayer->size()-2)->getVectNeuron()->size();
    //cout<<"tamaño de salida"<<sizeLast<<endl;
    vector<Neuron *> * VN= vectLayer->at(vectLayer->size()-2)->getVectNeuron();
...
void Network::loadDataNumbersLastLayer(string name, int a, vector< vector<double >> &training)
{
    training.resize(a);
    for(int i = 0 ; i< a ;i++)
        training[i].resize(getNumEntradas()-1);
...
void Network::saveWeithgs(string namefile)
{
    mat *A1= vectLayer->at(0)->getMat();
    //mat *A2 = vectLayer->at(1)->getMat();
    //mat * X = vectNeurontoMatrix( vectLayer->at(1)->getVectNeuron());
    bool status1 = A1->save(namefile+"_pesos1.mat");
    //bool status2 = A2->save(namefile+"_pesos2.mat", arma_ascii);
    //bool status3 = X->save(namefile+"layer.mat", arma_ascii);
